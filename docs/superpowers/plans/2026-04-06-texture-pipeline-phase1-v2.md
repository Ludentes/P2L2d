...
- `manifests/hiyori_atlas.toml` — exists (union-bbox placeholder, will be regenerated by Task 5)
...
### Task 6: Run `measure_regions.py` on Hiyori and commit real atlas (user-interactive)

This task requires the operator (you) to:
...
2. For each `group_NNN.png` in `/tmp/atlas_panels/`, apply the drawable-labeling playbook
   (see `docs/runbooks/drawable-labeling-playbook.md`).

3. Edit `manifests/hiyori_atlas.toml`:
   - Replace `label = "other"` with correct region names
   - Set `swap_strategy = "hue_shift"` for all hair regions
   - Remove `# REVIEW` comments after verification
...
```bash
git add manifests/hiyori_atlas.toml
git commit -m "feat(atlas): hiyori_atlas.toml — measured per-drawable UV bboxes"
```
